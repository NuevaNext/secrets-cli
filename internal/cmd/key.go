...
func init() {
	...
	keyRemoveCmd.Flags().BoolVarP(&forceKeyRemove, "force", "f", false, "Force remove without confirmation")
}
...
func runKeyRemove(cmd *cobra.Command, args []string) error {
	...
	if !Confirm(fmt.Sprintf("Are you sure you want to remove public key for %q?", email), forceKeyRemove) {
