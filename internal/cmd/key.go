...
func init() {
	...
	keyRemoveCmd.Flags().BoolVarP(&forceKey, "force", "f", false, "Force remove key without confirmation")
}
...
func runKeyRemove(cmd *cobra.Command, args []string) error {
	...
	// Validate to prevent path traversal and argument injection
	if err := validateName(email); err != nil {
		return err
	}
	...
	if !Confirm(fmt.Sprintf("Are you sure you want to remove the GPG key for %q?", email), forceKey) {
